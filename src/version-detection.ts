...
/**
 * Validate if the server version is at least the minimum required version
 * @param serverVersion - The detected server version
 * @param minimumVersion - The minimum required version (e.g., "4.0")
 * @returns boolean indicating if the server version is valid
 */
export function validateMinimumVersion(serverVersion: ServerVersionInfo, minimumVersion: string): boolean {
  const [minMajor, minMinor] = minimumVersion.split('.').map(Number);

  // Check if server version is greater than or equal to minimum version
  return (
    serverVersion.major > minMajor ||
    (serverVersion.major === minMajor && serverVersion.minor >= minMinor)
  );
}
